...
public class AccountWindow {
    ...
    // define a constructor of the account window
    public AccountWindow(String userName) throws JsonProcessingException {
        ...
        tablePane.setPreferredSize(new Dimension(400, 280));
        ...
        logOutButton.addActionListener(event -> SwingUtilities.invokeLater(LoginWindow::new));
...
//AdminPanel
        ...
        adminLogOutButton.addActionListener(event -> SwingUtilities.invokeLater(LoginWindow::new));
        ...
        adminLogOutButton.setAlignmentX(Component.CENTER_ALIGNMENT);


        if (isAdmin) {
            singletonWindow.setLayout(new GridBagLayout());
            singletonWindow.add(adminPanel, new GridBagConstraints());
        } else {
    ...
    public void addBookToCustomer(Integer bookID) {
        List<Integer> bookList = customer.getMyLibrary();
        if (findBookByIDRequest(bookID).equals("")) {
            alarmField.setText("Don't have this book!");
        } else {
